...
La plupart des touches de raccourci que Visual Studio éditeur de code utilise fonctionnent également dans l’expérience d’édition des scripts Office. Consultez les clés de raccourci suivantes.
...
L’écriture de types peut être facultative dans TypeScript, car l’inférence de type vous permet d’obtenir beaucoup de puissance sans écrire de code supplémentaire. La plupart du temps, le langage TypeScript est bon pour déduire les types de variables. Toutefois, dans certains cas, les scripts Office exigent que les déclarations de type soient explicitement définies si la langue ne parvient pas à identifier clairement le type. En outre, les scripts explicites `any` ou implicites ne sont pas autorisés dans Office Script. Plus d’informations à ce jour.
